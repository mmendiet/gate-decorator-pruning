...
class Block(nn.Module):
    def __init__(self, inp, outp, stride, tmp_ratio=1.0):
        ...
        # expand_ratio = 0.25
        layers = [
            # nn.Conv2d(inp, midp, 1, 1, 0, bias=False, ratio=[tmp_ratio, expand_ratio]),
            nn.Conv2d(inp, midp, 1, 1, 0, bias=False),
            # nn.BatchNorm2d(midp, ratio=expand_ratio),
            nn.BatchNorm2d(midp),
            nn.ReLU(inplace=True),

            # nn.Conv2d(midp, midp, 3, stride, 1, bias=False, ratio=[expand_ratio, expand_ratio]),
            nn.Conv2d(midp, midp, 3, stride, 1, bias=False),
            # nn.BatchNorm2d(midp, ratio=expand_ratio),
            nn.BatchNorm2d(midp),
            nn.ReLU(inplace=True),
        ...
        if not self.residual_connection:
            self.shortcut = nn.Sequential(
                ...
                nn.Conv2d(inp, outp, 1, stride=stride, bias=False),
                nn.BatchNorm2d(outp),
            )
...
class Model(nn.Module):
    def __init__(self, num_classes=1000, input_size=224, cfg=None):
